...
func Print(name string, r io.Reader) (string, error) {
	parsed := &Document{}
	if err := documentParser.Parse(name, r, parsed); err != nil {
		return "", err
	}
	...
	for i, policy := range parsed.Policies {
		p.printPolicy(policy)
		if i != len(parsed.Policies)-1 {
			p.write("\n")
		}
	}
